/*CImg<double> inTmp;
	for (int t=0;t<10000;t+=400){
		std::ostringstream oss;
		oss <<t;
		inTmp.load_cimg(const_cast<char*>(("/Users/pheas/QuetzalCoatl/Prog/DataBase/DataDNS/sqg/resultAug2013/tmp/bObsSst_" + oss.str() +".cimg").c_str()));
		inTmp.save_raw(const_cast<char*>(("/Users/pheas/QuetzalCoatl/Prog/DataBase/DataDNS/sqg/resultAug2013/tmp/bObsSst_" + oss.str() +".raw").c_str()));
	}
*/

#include "smctc.hh"
#include "PffuncsRayleigh.hh"
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
//smc::rng pRng;
        ...
        smc::sampler<cv_state> Sampler(lNumber, SMC_HISTORY_RAM);
...
// for (int k=0;k<6;k++) y->g[k]=pRng.Normal(0,sqrt(var_obs));
